...
- **Privacy.** Empirical Precision uses Google Analytics/Ads. Nothing you enter into EP is ever sent to Google. Your loads, firearms and range sessions stay on your device.
...
**Step 0. Sync the master database.** Go to **DB Management** and click **Sync Remote Repo**. This pulls the full library of bullet profiles, powders and factory cartridges into your local library. So you rarely have to enter a component by hand. It is safe to re-run. It merges by ID and never touches your personal records.
...
The Dashboard is reached via the logo and it is your launch pad. It lays out the six-step Getting Started workflow as clickable cards. It also offers one-tap buttons to **Sync the master database**, **open this User Guide** and **join the Discord**. It's the fastest way to orient a new install or jump back into the pipeline. The bottom of the page holds links to the **Documentation**, **Feedback** and **Models** references, plus contact and publisher information. The [Glossary](#16-glossary) below is the source of truth for any label that seems ambiguous.
...
**Varytita: drop chart, DOPE card and field solution.** Charts elevation, wind and velocity from the RK4 trajectory engine in a Quick or Advanced tier, with truing, cant, look angle and a **Maximum Point Blank Range (MPBR)** solver. Its DOPE tier prints pocket range cards with wind brackets, as a PNG or CSV. Its Field tier solves a list of targets in large type, with lead for movers and a reticle view. *Why:* a field-ready come-up card built from your exact load and conditions.
...
| **Cartridges** | Chamberings tied to a diameter, with SAAMI specs and both pressure limits: SAAMI in PSI and C.I.P. in bar. The two are different standards, not two units of one number |
...
- **Sync Empirical Precision Database → Sync Remote Repo** pulls the whole master component library of bullets, powders and cartridges. It merges by ID and your personal records are untouched. Run it on first setup and whenever you want the latest reference data.
- **Backup & Transfer** is your backup and your way of keeping two browsers in step, such as a desktop and a tablet. **Export Backup** writes one personal DB file (PDBF) holding your firearms, loads, targets, sessions, chronograph strings, saved scenarios and any custom components. The reference library is left out since every install syncs it. On a phone or tablet it is two taps: **Prepare Backup** builds the file and a **Share Backup** button then opens the device's share sheet so the file can go straight to AirDrop, Nearby Share, Files or a cloud folder. The share sheet has to open directly from a tap. That is why the file is prepared first. The shared copy is named `.txt` because phone share sheets refuse `.json`. It is the same file and Import reads either name. On a desktop without a share sheet **Export Backup** downloads in one tap. **Import Backup** merges that file into the device. Every record carries the time it was last written. The newer copy wins. A record deleted on one device is deleted on the other when the file arrives. Records the device made itself are untouched. Images are rebuilt automatically. Export the tablet and import on the desktop and both end up the same. Session files (EPSF) from the Sessions page are a different format and are imported there.
- **Advanced Database Settings:**
  - **Active Table** scopes the wipe and the records browser to the whole database or one table.
...
**Back up:** **Backup & Transfer → Export Backup** → save or share the `.json` somewhere safe such as cloud storage and an external drive.

**Restore:** **Backup & Transfer → Import Backup** → choose your PDBF → merge. Newer copies win and deletions carry across. Firearm velocity offsets ride along automatically.
...
**Read that as a direction rather than a measurement.** Published load data cannot separate a primer effect from the manual that published it. Nearly every source uses one primer for its entire book and across the whole corpus only a handful of loads differ by primer alone. So the model spreads primers around a calibrated centre using their brisance energy. That centre was solved for numerically. So the average load lands exactly where the calibration put it. The direction is physics. The size is a display choice.
...
### 14.2 Varytita: the drop chart, the DOPE card and the field solution
...
| **Field** | Advanced plus a list of targets, each solved in large type with lead and a reticle view |

The tier only decides what is on screen. Every field keeps its value in every tier and feeds every run. A field Quick hides still counts. Until you change them, the hidden fields sit at the standard defaults: 1:10 twist, 1.5 in sight height, 59 °F, 29.92 inHg, 50% humidity, sea level and latitude 45°.
...
**Saved Profile / Save / Export / Import / Delete.** A profile is the whole page: every field, the selections and your truing history. **Save** stores it under the name typed beside it and the same name updates it. **Export** writes a file you can keep or move to another device, and **Import** reads one back. Loading a profile restores its trued MV and drag scale over whatever the selected bullet would fill in. A profile saved by an older build is refused with a message. Re-enter it and save it again.

#### Selections

**Session / Firearm / Caliber / Cartridge / Bullet Manufacturer / Bullet Name.** The fast way to fill the profile.
- A **session** fills the MV from your recorded velocities, or from the internal ballistics engine when none were recorded. It also fills the bullet and the firearm, the session's temperature, pressure and altitude, its zero distance and range units, and the zero offsets from your marked shots.
...
**Bullet Diameter (in) / Bullet Weight (gr) / Muzzle Velocity (fps) / Ballistic Coefficient (BC).** The four required fields. The chips above the RUN button turn green as each is filled.

**Ballistic Coefficient (BC).** **The field that shapes the whole chart.** **Raise it** and every drop number shrinks. **Change it when:** you've measured it in Rheos. An optimistic advertised BC produces a card that's close at 300 and increasingly wrong past 700. That is the classic "my dope stops matching past 600" complaint.

**Drag Model.** G7 for boat-tail bullets and G1 for flat-base ones. It must match the family the BC was published in. G7 numbers are roughly half the G1 number for the same bullet. For a library bullet with its own drag curve, the note under the box says what the BC rests on and where the curve came from. That bullet then flies its own curve in either family, so switching G1 and G7 changes the number shown and not the trajectory. **Edit curve →** opens the bullet's curve in the editor on the Components page (section 12).
...
**Adjust MV for powder temperature.** (Advanced.) A **slope** in fps per °F and the temperature the MV above was **measured at**. Ticked, the run flies the MV the slope gives at today's temperature. When your sessions hold at least two chronographed strings of this load 10 °F or more apart, the fitted slope is shown under the box so you can copy it in.

**Zero Distance (yd).** The range where the rifle is zeroed. **This is the anchor for every row.** The MPBR chart replaces it with the zero it calculates.

**Twist Rate (1:X) / Twist Direction.** (Advanced.) Feed **stability** and **spin drift**, and the chart includes both. At 1000 yd spin drift is several inches and always the same way. So leaving twist wrong bakes a constant lateral error into the chart. Right-hand twist drifts right.

**Sight Height (in).** (Advanced.) Bore centre to scope centre. Wrong here means a chart that's fine far out and wrong up close.

**Bullet Length (in).** (Advanced.) Feeds stability, spin drift and aerodynamic jump. Left blank, those use an assumed length and the Sg chip is starred.

**Zero Offset X / Y (in).** (Advanced.) Where your group actually sits at the zero distance, from your measured point of impact. **Leave at 0** for a true zero. A session fills these from its marked shots.

**Drag Scale by Mach.** (Advanced.) Multipliers on the drag curve, each pinned at a Mach number. Empty is the curve as shipped. One point scales the whole curve. Two or more run in a straight line between their Mach numbers and hold flat beyond them, so the transonic band can be corrected without moving the supersonic one. Truing adds the points (below). **+ point** adds one by hand if you already know yours.
...
Quick shows the wind speed only. Advanced shows the rest.
...
**Temperature (°F) / Pressure (inHg) / Pressure Type / Humidity (%) / Altitude (ft).** The air, in weather mode. **Pressure Type** says how to read the pressure: a weather report gives **barometric** pressure corrected to sea level, which the chart corrects to your altitude. A Kestrel reading on the spot is **station** pressure and is used as it is.

**Wind Speed (mph) / Wind From.** The wind the Wind column is computed for, as a **clock on the line of fire**: 12 is a headwind, 3 o'clock blows from the right, 6 is a tailwind and 9 o'clock blows from the left. Many shooters build the card at a **10 mph, 3 o'clock** wind and then scale in their head since half the wind is half the hold. That's a good default.
...
**Zeroed in different conditions.** Tick it when the rifle was zeroed on a different day. Enter that day's temperature, barometric pressure and altitude. The zero is flown in that air and at that powder temperature, and today's shot on the same angles. Left off, the rifle is taken as zeroed in today's air. Either way the zero is solved on a level range, upright and in still air. The MPBR chart always zeroes in today's air.

#### Chart Settings

**Regular Steps / Specific Stops.** (Advanced.) How the rows are chosen.
- **Regular Steps** gives a row every **Increment** out to the **Max Range**. Quick offers 25, 50 or 100 yd. Advanced takes any step.
...
**Elevation & Wind Units.** Tick any of inches (centimetres in meters mode), MOA and MIL. Each ticked unit adds an Elev and a Wind column. Changing them re-shapes the table without a re-run.

**Maximum Point Blank Range chart.** Ticked, the run solves the zero that keeps the bullet inside the vital zone for the longest span and **replaces your zero distance with it**. It marks the Apex, the Far Zero and the MPBR on the table and draws the vital-zone pipe on the chart.

**Range Units.** (Advanced.) Yards or meters for the whole page.

**Turret Click Value.** (Advanced.) Your scope's click, such as 1/4 MOA or 0.1 MIL. **It also sets the turret's unit**, so pick MIL clicks for a MIL scope. Getting the unit wrong is the fastest way to a dangerous miss because MIL and MOA numbers are similar in size and easy to confuse under stress. The Clicks column counts in it.

**MPBR Vital Zone (in).** (Advanced.) The target the MPBR solver must stay inside: 4 for varmints, 6 or 8 for deer and 10 for elk. **This is the only input to point-blank zero that matters.** **Lower it** and the point-blank span shrinks sharply. That is the trade-off, and seeing it quantified is the point of the tool.

#### Truing

(Advanced.) Shoot a group at a known range and enter where it landed, then let the chart solve the number that explains it.

**Range / Observed drop / Unit.** The **observed drop** is the total from the line of sight: the chart's Elev for that range plus however low (+) or high (−) the group printed.

**True MV / True Drag.** A miss in the supersonic band is a muzzle-velocity error. A miss in the transonic band is a drag error. So **true MV first** with a group inside the **Mach 1.2** range, then **true Drag** near the **Mach 0.9** range. Run the chart first and the card names both ranges. **True MV** writes the solved velocity into the MV field. **True Drag** pins a drag point at the Mach the bullet had at that range. The first drag point after an MV truing also gets a point of 1.0 at the MV truing's Mach, so fixing the far groups cannot undo the near ones. True Drag again at another range to add a point there. The same band trued twice replaces its point. It warns when the answer hits the edge of the allowed band, 0.85 to 1.15. That usually means the MV, the BC or the drag family is wrong. Each truing is logged with its Mach and saves with the profile. Firing east or west, set the azimuth first: the vertical Coriolis term is real and is in the chart.

#### Reading the results

**The chips** above the chart:
- **DA:** density altitude.
- **Sg:** stability. It turns amber under 1.4, and a star means the bullet length was assumed.
...
**Why It Might Not Match.** (Advanced.) Eight checks on the inputs behind most "the app is wrong" reports: sight height, pressure type, a BC in the wrong drag family, a typed rather than measured MV, the default twist, a blank bullet length, a zero past 300 yd and nothing trued yet. Each flag says what to fix.

#### DOPE tier

**The card** is the white printout. Its Elev columns follow the units you ticked, and the turret's unit carries the click count in brackets. **Strategy** reads Aim Center inside the MPBR and Dial beyond it. The MPBR block gives the near zero, the far zero and the maximum point-blank range. The warning under it matters: the chart is only valid with the rifle zeroed at one of those two distances.

**Wind Bracket (mph).** A comma list such as `5, 10, 15`. The card grows one Wind column per speed, in the turret's unit, at the wind direction above, with R or L for the side to hold. Blank keeps a single wind column.
...
**Targets.** One line per target. Each has its own **slant range**, as a rangefinder reads it, its own **bearing** and its own **look angle**. A blank bearing is the firing azimuth. Solutions update as you type, with no RUN button.
...
**Dial elevation, hold wind / Hold everything.** Whether the elevation goes on the turret or on the reticle. Dialing rounds to whole clicks and the reticle shows what is left over.
...
**Bullet Length (in).** **Auto-fills from:** the bullet record. **Feeds:** the stability factor behind spin drift and aerodynamic jump. Leave it empty and the engine assumes a length of 4.5 calibres. Every tool that flies a trajectory reads twist, sight height, zero and the atmosphere through one shared builder with one table of defaults. So a blank field means the same thing on every tab.
...
**Muzzle Velocity (fps).** The average speed at the muzzle. **Auto-fills from:** the session's chronograph string if there is one, else velocities on its marked target, else the Ignition estimate with your rifle's saved velocity offset applied. Every Heurisko tool resolves it the same way and shows the same number. **Use your measured mean if you have it.**
...
| What do I dial? | **Varytita** (DOPE tier) | Ballistic Coefficient, Turret Click Value |
| Where can I hold dead-on? | **Varytita** (MPBR chart) | MPBR Vital Zone |
| What do I dial and hold for this stage, or this mover? | **Varytita** (Field tier) | Targets, Wind From, Dial or Hold |
